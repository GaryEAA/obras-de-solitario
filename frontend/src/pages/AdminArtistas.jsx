...
import { getArtistas, crearArtista, editarArtista } from '../api/obras'
...
export default function AdminArtistas() {
  ...
  const guardar = async () => {
    ...
    try {
      if (modal === 'crear') {
        await crearArtista(editando)
      } else {
        await editarArtista(editando.id, editando)
      }
      setMensaje({ tipo: 'ok', texto: modal === 'crear' ? 'Artista creado.' : 'Artista guardado.' })
      cerrar()
      cargar()
    } catch {
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto rounded-xl border border-line">
            <table className="w-full text-sm">
              ...
              <tbody>
                {artistas.map((a) => (
                  <tr key={a.id} className="border-t border-line hover:bg-surface transition-colors">
                    <td className="px-4 py-3 font-medium text-paper">{a.nombre}</td>
